...
plot_umap_color <- function(
  ...
  no_margin <- TRUE
  width <- if (no_margin) 2000 else 2800
  height <- if (no_margin) 1000 else 2000
  xlim <- if (no_margin) c(-10, 9) else c(-10, 10)
  ylim <- if (no_margin) c(-5, 7) else c(-10, 10)
  xlab <- if (no_margin) "" else "UMAP.1"
  ylab <- if (no_margin) "" else "UMAP.2"

  cex <- ifelse(plotting_df$Color == "gray", 0.2, 0.5)
  col <- plotting_df$Color
  col[col == "gray"] <- "gray80"
  png(filename = paste0("UMAP_", title, ".png"),
      width = width, height = height, res = 400)
  if (no_margin) par(mar = c(0, 0, 0, 0))
  ...
  main <- if (no_margin) "" else paste("UMAP cla Cluster ", title)
  title(main = main)
  dev.off()
}
...
plot_slingshot <- function(
  ...
  plot(
    x = plotting_df$UMAP.1,
    y = plotting_df$UMAP.2,
    col = ifelse(test_mode, plotting_df$Color, "gray85"),
    pch = 20,
    cex = 0.2,
    axes = !no_margin,
    las = 1,
  ...
  if (length(lins) > 0) {
    ...
    for (Lineage in names(lins)) {
      ...
      points(color_curve$pos, col = color_curve$col, pch = 20, cex = 3)
    }
  }
  ...
  main <- ifelse(no_margin, "", paste("UMAP cla Cluster ", title))
  title(main = main)
...
project_umap[!(project_umap$Color == "#FF7F0F" |
                 project_umap$Color == "#6D289D"), "Color"] <- "gray"

#Keep BlRe
#project_umap[!(project_umap$Color == "#1F77B4" |
...
colnames(new_cla_cluster)[22] <- "Color"

#20231202 change light pink and light red to pink and red
new_cla_cluster[new_cla_cluster$Color == "#A396B6", "Color"] <- "#6D289D"
new_cla_cluster[new_cla_cluster$Color == "#D99694", "Color"] <- "#D62728"
#unknown cell type to gray
new_cla_cluster[new_cla_cluster$Color == "#4B4B4B", "Color"] <- "gray"
new_cla_cluster[new_cla_cluster$Color == "#9B9B9B", "Color"] <- "gray"
#keep only OrPu
new_cla_cluster[!(new_cla_cluster$Color == "#FF7F0F" |
                    new_cla_cluster$Color == "#6D289D"),
                "Color"] <- "gray"
#keep BlRe
#new_cla_cluster[!(new_cla_cluster$Color == "#1F77B4" |
#                    new_cla_cluster$Color == "#D62728"),
#                "Color"] <- "gray"
...
#Get Cla cells that similar to Ptr
...
plot_slingshot(cla2,
               lins = cla2_lin,
               lins_last = cla2_lin_cut,
               title = "Cla_combined_PtrLike")


#Get Cla cells that similar to Lch
...
plot_slingshot(cla3,
               lins = cla3_lin,
               lins_last = cla3_lin_cut,
               title = "Cla_combined_LchLike")


#combine ray cell from both PtrLike and LchLike
#resign clusters
cla4 <- cla2
