...
plot_list <- lapply(valid_genes, function(gene_name) {
  df <- counts_cluster_ss %>% select(UMAP.1, UMAP.2, expr = all_of(gene_name))
  ...
  if (all(df$expr == 0, na.rm = TRUE) || sd(df$expr, na.rm = TRUE) < 1e-6) {
    ...
    df <- df %>% mutate(expr_clip = pmin(pmax(expr, q15), q95)) %>% arrange(expr_clip)
...
ggsave(file.path(output_dir, "UMAP_individual_genes_Cla_patchwork.png"),
       combined_plot, width = 18, height = 13, dpi = 300)




############################## UMAP per gene (not sum) per plot
##############################
##############################
