setwd("/Users/joweihsieh/Dropbox/YCL/Single_Cell_Cla/LCM_proteomes/20250522/report")
...
df <- read.csv("/Users/joweihsieh/Dropbox/YCL/Single_Cell_Cla/LCM_proteomes/20250522/1140650-1140654_Cla-bio(All_Protein)_20250521_Peptide Quant Report.csv")
colnames(df)[c(8:12)] <- c("Bio1_O_tech2", "Bio1_I_tech2", "Bio1_O_tech3", "Bio1_I_tech3", "Bio1_O_tech1")
Cla_Proteomes <- df[,c(1,7:12)]
...
plot_line_with_CI_by_cluster_grouped <- function(subset_df, outdir, stage_map, stage_colors) {
  unique_bios <- unique(subset_df$BioColumn)
  ...
  for (bio in unique_bios) {
    ...
    p <- ggplot(bio_df, aes(x = Stage, y = mean)) +
      geom_ribbon(aes(ymin = lower, ymax = upper, fill = Stage), alpha = 0.3) +
      ...
      theme(axis.text.x = element_text(size = 18),
            axis.text.y = element_text(size = 18),
            legend.position = "none") +
      ggtitle(paste0("Stage Mean ± 95% CI: ", bio)) +
      xlab("Stage") + ylab("Correlation")

    ggsave(filename = paste0(outdir, "/", bio, "_stage_grouped_mean_CI_lineplot.png"),
           plot = p, width = 6, height = 4, dpi = 300)
  }
}
...
process_single_bio_analysis <- function(bio_col,
  ...
  if (exists("Cla_colors_Df2") && "SpBarcode" %in% colnames(data_output)) {
    ...
    #plot_line_with_CI_by_cluster_grouped(cluster_long_df, cluster_dir, group_map, group_colors)
